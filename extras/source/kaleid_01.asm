; This is an annotated version of the KALEIDOSCOPE
; program for CHIP-8. The original code can be found in
; hexadecimal in the COSMAC VIP Instruction manual
; (included in the extras/docs folder). As a binary 
; CHIP-8 executable it can be found under various names
; in every CHIP-8 repository.

LBL0200:
	LD V0, #00
	LD V3, #80 ; index to buffer 0280..02FF
	LD V1, #1F ; half the screen width - 1?
	LD V2, #0F ; half the screen height -1?
...
;
; In the original, V1 can range from 0 to 3F (0-63)
; and V2 from 0 to 1F (0-31).
;
; V1 can contain from $.... .... to $..11 1111
; V1 AND $111. .... results in either #00 or #20
; where 0 means, V1 indexes the left side of screen,
; #20 means, indexes the right side.
;
	LD VA, #E0		
	AND VA, V1		; VA = left or right
;
; V1 AND $...1 1111 results in 0..1F
;
	LD VB, #1F		; V1 gets X_coord for left or
	AND V1, VB		; right half

	SE VA, #00		; if it is #20==Right side,
	ADD V2, #01		; .. Y_coord += 1

; V2 can be from 00..1F
; AND that with #F0 you get either #10 for bottom half,
; or #00 for top half.

	LD VA, #F0
	AND VA, V2		; VA = $10 for bottom, 0 for top
	LD VB, #0F		; V2 gets Y_coord for either
	AND V2, VB		; upper or lower

	SE VA, #00 		; if it is $10==bottom half,
	ADD V1, #01		; .. X_coord += 1

	LD VB, #1F		; in case the increment flipped it,
	AND V1, VB		; force V1 back to left side

	DRAW V1, V2, 1 	; draw in the upper left quad
	LD VA, V1		; save the X_coord

	LD VB, #1F		; get the 32-complement of the
	SUB VB, V2		; Y_coord in VB

	DRAW VA, VB, 1	; draw the lower left quad

	LD VA, #3F		; get the 64-complement of the
