...
Note that if an assembler source file contains accented characters and symbols
that are not in the ASCII set, it will be treated as a binary executable
when it is opened, with results that may be amusing but will not be useful.
...
String data is ASCII characters enclosed in single quotation marks, `'LIKE THIS'` (Strings are used only in the **DA** directive). Use a pair of quotation marks to include a single quotation mark in the string: `'THAT''S HOW'`.
...
* Binary when preceded by a dollarsign, `$00011011`.
  ...
  When writing binary, you may use a decimal point in place of a zero, which makes it easier to define sprite values: `$...11.11`.
...
**CALL** *address*  
Push PC on the call stack; jump to specified address.
The call stack is limited to 12 levels.
A thirteenth call forces an error stop. (2aaa)
...
**SKP** **V***s*  
Skip the following instruction if the key (0-16) specified by **V***s* is down (being pressed). This only samples the keypad; the pressed key (if any) is not cleared. (Es9E)

**SKNP** **V***s*  
Skip the following instruction if the key (0-16) specified by **V***s* is not pressed. This only samples the keypad; the pressed key (if any) is not cleared. (EsA1)
...
**LD** **V***t*, **K**  
Wait for a key to be pressed, if one is not already being pressed.
Put the code for that key (0..16) in **V***t*.
Continue to wait until the key is released before returning.
(Note that this ties up the emulator until a key is entered and released on the display,
...
If **V***s* <= **V***t*, **VF** is set to 1.
If **V***s* > **V***t* there is underflow and **VF** is set to 0. (8ts5)

**SUBN** **V***t*, **V***s*  
Subtract the contents of **V***t* from **V***s* and put the result in **V***t*.
If **V***t* <= **V***s*, **VF** is set to 1. If **V***t* > **V***s* there is underflow and **VF** is set to 0. (8ts5)

**SHR** **V***t*, **V***s*  
Shift the contents of **V***s* right 1.
The result is stored in **V***t* (*see note below*).
The shifted-out bit, 0 or 1, is set as the value of **VF**.
...
**SHL** **V***t*, **V***s*  
Shift the contents of **V***s* left 1.
The shifted-out bit, 0 or 1, is set as the value of **VF**.
The result is stored in **V***t* (*see note below*).
To gain the effect of shifting the contents of a register within that same register,
specify the same register twice, for example `SHL v3, v3 ; shift v3 left`. (8tsE)
...
**ADD** **I**, **V***s*  
Add the contents of **V***s* to **I**.
Note that this can potentially set **I** to an address outside memory,
that is, >4095). (Fs1E)
...
This instruction can be used to load the single register **V0**
from memory addressed by **I**: `LDM V0`.
Another typical use for load-multiple is to load the three digit
values stored by **STD** into registers **V0**, **V1** and **V2**: `LDM V2`. (Ft65)
...
**DRAW** **V***x*, **V***y*, *len*  
Draw the sprite currently addressed by **I** on the screen.
The X-coordinate of the upper left corner of the sprite is taken from **V***x*; the Y-coordinate from **V***y*.
The value of *len* specifies how many bytes are in the sprite, from 1 to 15.
If *len* is 0, **I** must address an SCHIP 16x16 sprite, comprising 32 bytes in all. (Dxyn)

If the sprite extends past the right side or bottom row of the display,
some pixels "wrap" to the left side or top edge.
